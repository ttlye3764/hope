...
public class CarController {
	...
	public ModelAndView carView(HttpSession session, ModelAndView andView, Map<String, String> params, String car_no,
			MyCarVO carInfo) throws Exception {
		MemberVO memberInfo = (MemberVO) session.getAttribute("LOGIN_MEMBERINFO");
		String mem_no = memberInfo.getMem_no();
	...
	public String insertMycar(MyCarVO carInfo, HttpSession session) throws Exception {
		...
		String[] no = carInfo.getCar_no().split(" ");

		if (no.length > 1) {
			String car_no = no[0] + no[1];
