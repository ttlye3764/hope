...
class BriteChart {
  ...
  /**
   * Creates the horizontal bar chat
   * @param {*} elementSelector
   * @param {*} data
   */
  ...
  init() {
    ...
    function drawCharts() {
      ...
      if ($('.bar-container').length > 0) {
        self.createBarChart('.bar-container', barData);
      }

      if ($('.bar-container-horizontal').length > 0) {
        self.createHorizontalBarChart('.bar-container-horizontal', barData);
      }

      if ($('.line-container').length > 0) {
        self.createLineChart('.line-container', lineData);
      }

      if ($('.donut-container').length > 0) {
        self.createDonutChart('.donut-container', donutData);
      }

      if ($('.brush-container').length > 0) {
        self.createBrushChart('.brush-container', brushData);
      }

      if ($('.step-container').length > 0) {
        self.createStepChart('.step-container', stepData);
      }
    }
