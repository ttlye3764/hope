...
$(function(){
    ...
    $('form[name=boardView]').on('submit', function(){
        ...
        $(this).attr('action','${pageContext.request.contextPath}/admin/board/updateBoardInfo.do?bd_no=' + bd_no + '&re_no=${re_no}' + '' + '&currentPage=${currentPage}' + '&search_keyword=${search_keyword}' + '&search_keycode=${search_keycode}');
    ...
    // 삭제버튼
  	$('#deleteBtn').on('click', function(){
		//alert('삭제');
		var bd_no = $('input[name="bd_no"]').val();
		$(location).attr('href','${pageContext.request.contextPath}/admin/board/deleteBoardInfo.do?bd_no=' + bd_no + "&bd_division=${bd_division}");
  	 });  
...
//         }else{
			$(location).attr('href','${pageContext.request.contextPath}/admin/board/boardList.do?currentPage='+ currentPage + '&search_keyword='+ search_keyword + '&search_keycode=' + search_keycode + "&bd_division=${bd_division}");
//         }
	...
	$('#Btn').on('click', function(){
		...
		$(location).attr('href', '${pageContext.request.contextPath}/admin/board/replyForm.do?bd_division=${bd_division}');
...
function insertReplyA(bd_no){
	...
	$.ajax({
   	 	async    : false,
        url     : '${pageContext.request.contextPath}/admin/board/insertBoardReply.do',
        type    : 'post',
        dataType : 'json',
        data : {'re_content':re_content,'bd_no':bd_no,'mem_no': '${LOGIN_MEMBERINFO.mem_no}'},
...
function replyList(){
	$.ajax({
		url : '${pageContext.request.contextPath}/admin/board/replyList',
		data : {
			bd_no : $('#bd_no').val()
		},
		success : function(result){
...
function fileDown(fileName, fileNo, fileBdNo) {
	$(location).attr('href','${pageContext.request.contextPath}/admin/board/fileDownload.do?fileName='+ fileName + '&fileNo='+ fileNo + '&fileBdNo='+ fileBdNo);
}
...
function updateReply2(bd_no, re_no, index){
	...
	$(location).attr('href','${pageContext.request.contextPath}/admin/board/updateBoardReply.do?re_content='+ re_content + '&bd_no=' + bd_no + '&re_no=' + re_no + '&bd_division=${bd_division}');		
}
...
function deleteReplyA(bd_no, re_no){
	...
	$(location).attr('href', '${pageContext.request.contextPath}/admin/board/deleteBoardReply.do?bd_no=' + bd_no + '&re_no=' + re_no + '&bd_division=${bd_division}');
}
...
<div class="container">
			...
			<div class="row mt-5">
						...
						<div class="col-md-2 text-center" style="margin-left:800px; float:left; display:inline-block;">
